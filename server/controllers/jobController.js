...
const createJob = async (req, res) => {
  try {
    const { user_id, vehicle_id, title, symptoms, priority, appointment_date, estimated_hours, estimated_days, image_url } = req.body
    ...
    // Create an initial timeline event with the image if provided
    if (image_url) {
      await pool.query(
        `INSERT INTO public.job_status_history (job_id, status, changed_by, notes, image_url)
         VALUES ($1, $2, $3, $4, $5)`,
        [newJob.job_id, 'pending', user_id, 'Attached image to request', image_url]
      )
    }
...
const updateJobStatus = async (req, res) => {
  try {
    const { id } = req.params
    const { status, changed_by, notes, image_url } = req.body
    ...
    await pool.query(
      `INSERT INTO public.job_status_history (job_id, status, changed_by, notes, image_url)
       VALUES ($1, $2, $3, $4, $5)`,
      [id, normalizedStatus, changed_by || null, notes || null, image_url || null]
    )
...
const getJobHistory = async (req, res) => {
  try {
    const { id } = req.params
    const result = await pool.query(
      `SELECT h.history_id, h.job_id, h.status, h.changed_at, h.notes, h.image_url,
...
const getApprovedJobsForAI = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT j.*,
              u.full_name AS customer_name,
              v.make, v.model, v.registration_number,
              q.amount AS approved_quote_amount
       FROM public.jobs j
       JOIN public.users u ON u.user_id = j.user_id
       JOIN public.vehicles v ON v.vehicle_id = j.vehicle_id
       JOIN public.quotes q ON q.job_id = j.job_id
       WHERE q.status = 'approved' AND j.status != 'completed'
       ORDER BY j.created_at DESC`
    )
    res.json(result.rows)
  } catch (error) {
    res.status(500).json({ message: 'Failed to fetch approved jobs', error: error.message })
  }
}
...
const cancelJob = async (req, res) => {
  try {
    ...
    // Only the job owner can cancel, and only if pending or diagnosed
    const jobCheck = await pool.query(
      `SELECT job_id, status, user_id FROM public.jobs WHERE job_id = $1`,
      [id]
    )
    ...
    if (String(job.user_id) !== String(user_id)) {
      return res.status(403).json({ message: 'You can only cancel your own jobs' })
    }
    if (!['pending', 'diagnosed'].includes(job.status)) {
      return res.status(400).json({ message: 'Can only cancel pending or diagnosed jobs' })
    }

    const result = await pool.query(
      `UPDATE public.jobs SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE job_id = $1 RETURNING *`,
      [id]
    )

    await pool.query(
      `INSERT INTO public.job_status_history (job_id, status, changed_by, notes)
       VALUES ($1, 'cancelled', $2, 'Job cancelled by customer')`,
      [id, user_id]
    )
